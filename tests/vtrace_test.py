...
class ActionLogProbsTest(unittest.TestCase):
    def test_action_log_probs(self, batch_size=2):
        ...
        policy_logits = list(
            map(
                lambda num_actions: _shaped_arange(seq_len, batch_size, num_actions),
                action_shape,
            )
        )
        actions = list(
            map(
                lambda num_actions: np.random.randint(
                    0, num_actions, size=(seq_len, batch_size, 1), dtype=np.int64
                ),
                action_shape,
            )
        )
        ...
        action_log_probs_tensor = vtrace.action_log_probs(
            list(map(lambda array: torch.from_numpy(array), policy_logits)),
            torch.from_numpy(np.concatenate(actions, axis=-1)),
        )

        # Ground Truth
        # Using broadcasting to create a mask that indexes action logits
        action_index_mask = list(
            map(
                lambda action, num_actions: action == np.arange(num_actions),
                actions,
                action_shape,
            )
        )
        ...
        ground_truth_v = 0
        for logits, mask in zip(policy_logits, action_index_mask):
            ground_truth_v += index_with_mask(np.log(_softmax(logits)), mask)
        ...
        values = {
            "actions": np.concatenate(
                list(
                    map(
                        lambda num_actions: np.random.randint(
                            0, num_actions - 1, size=(seq_len, batch_size, 1)
                        ),
                        action_shape,
                    )
                ),
        ...
        values.update(
            {
                k: list(
                    map(
                        lambda num_actions: torch.from_numpy(
                            _shaped_arange(seq_len, batch_size, num_actions)
                        ),
                        action_shape,
                    )
                )
                for k in ["target_policy_logits", "behavior_policy_logits"]
            }
        )
